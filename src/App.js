...
const DEFAULT_PROFILE = {
  ...
  role: "AI Specialist, Virtual Assistant & Operations Expert",
  location: "Ibadan, Nigeria",
  services: "AI workflow automation, no-code tool implementation (Notion/Zapier/Make), virtual assistance, operations optimization, SOPs, process documentation",
  targetMarket: "Startups, SMEs, and growing businesses that need help with operations and AI",
  linkedin: "https://www.linkedin.com/in/timileyin-ajuwon-53075aa8/",
};

/* ── Dynamic Prompts ────────────────────────────────────────────────────── */
const buildAgentPrompt = (p) => `You are an expert B2B outreach specialist working for ${p.name || "a freelance specialist"}${p.role ? ` — ${p.role}` : ""}${p.location ? ` based in ${p.location}` : ""}.
...
4. Soft CTA — naturally offer that ${p.name || "I"} can handle this if they prefer to delegate

Tone: Warm, confident, peer-to-peer. Like a smart friend giving free advice, not a vendor pitching.

${p.services ? `Services offered: ${p.services}.` : ""}
${p.targetMarket ? `Target market: ${p.targetMarket}.` : ""}

IMPORTANT RULES:
- Do NOT include any citation markers, reference tags, or source annotations
...
{
  "companyOverview": "2-3 clean sentences about what the company does. No citations.",
  "painPoints": [
    {
      "category": "AI/Tech OR Virtual Assistant OR Operations",
      "point": "Specific pain point headline",
      "signal": "One clean sentence explaining what signals this pain point. No citations."
    }
  ],
  "primaryPainPoint": "The single most compelling pain point",
  "emailSubject": "A compelling subject line",
  "emailBody": "Full email body, 150-200 words, signed off as ${p.name || "me"}. No citations.",
  "contacts": [
    {
      "role": "Job title",
...
const buildFollowUpPrompt = (p) => `Write a brief follow-up email for ${p.name || "a specialist"}${p.role ? ` (${p.role})` : ""}. The prospect has not replied to the original outreach.
...
const SYS_CONTACTS = `You are a B2B contact research specialist. Find the most relevant decision-makers at this company for outreach.
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div className="anim" style={{ maxWidth: 440, width: "100%", position: "relative" }}>
        ...
        <div className="card" style={{ padding: 24 }}>
          ...
          <input type="password" className="inp" value={k} onChange={e => setK(e.target.value)} onKeyDown={e => e.key === "Enter" && valid && onKey(k.trim())} placeholder="sk-ant-api03-..." style={{ marginBottom: 14 }} autoFocus />
          <button className="btn btn-primary" onClick={() => valid && onKey(k.trim())} disabled={!valid} style={{ width: "100%", padding: "11px 20px", fontSize: 14 }}>
            Get started →
          </button>
...
/* ── Agent Panel ─────────────────────────────────────────────────────────── */
function AgentPanel({ apiKey, profile, onSent, templates }) {
  ...
  const run = useCallback(async () => {
    ...
    try {
      ...
      const parsed = await callClaude(apiKey, buildAgentPrompt(profile), `Research this company and generate personalized outreach: "${company}". Be specific. Return only clean JSON with no citations or reference markers.`, true);
      await tick(2);
...
/* ── Batch Panel ─────────────────────────────────────────────────────────── */
function BatchPanel({ apiKey, profile, onBatchDone }) {
  ...
  const runBatch = async () => {
    ...
    for (let i = 0; i < companies.length; i++) {
      ...
      try {
        const r = await callClaude(apiKey, buildAgentPrompt(profile), `Research and generate outreach for: "${companies[i]}". Keep it concise. Return only clean JSON.`, true);
...
/* ── Tracker Panel ───────────────────────────────────────────────────────── */
function TrackerPanel({ apiKey, profile, log, setLog, onGoAgent }) {
  ...
  const genFU = async (entry) => {
    ...
    try {
      const r = await callClaude(apiKey, buildFollowUpPrompt(profile), `Company: ${entry.company}\nOriginal subject: ${entry.subject}\nOriginal email:\n${entry.emailBody}\n\nReturn only clean JSON.`);
...
/* ── Settings Panel ──────────────────────────────────────────────────────── */
function SettingsPanel({ profile, setProfile, onReset }) {
  ...
  const fields = [
    { key: "name", label: "Your Full Name", placeholder: "e.g. John Doe", hint: "Used to sign off your emails and personalize outreach" },
    { key: "role", label: "Your Role / Title", placeholder: "e.g. AI Automation Specialist", hint: "Shows in emails and the app footer" },
    { key: "location", label: "Your Location", placeholder: "e.g. Lagos, Nigeria", hint: "Used to add local context to your outreach" },
    { key: "linkedin", label: "LinkedIn Profile URL", placeholder: "https://linkedin.com/in/yourname", hint: "Linked in the app footer" },
  ];

  const textareas = [
    { key: "services", label: "Your Services", placeholder: "e.g. AI workflow automation, virtual assistance, Notion setup, process documentation...", hint: "What you offer — used to tailor the outreach email" },
    { key: "targetMarket", label: "Your Target Market", placeholder: "e.g. Startups and SMEs in Nigeria and West Africa that need operations support", hint: "Who you're reaching out to — helps Claude focus pain points" },
  ];

  return (
    <div className="col anim">
      {/* Profile incomplete warning */}
      {(!profile.name || !profile.services) && (
        <div style={{ background: "rgba(234,179,8,.08)", border: "1px solid rgba(234,179,8,.25)", borderRadius: 10, padding: "14px 18px", display: "flex", gap: 10, alignItems: "flex-start" }}>
          <span style={{ fontSize: 18, flexShrink: 0 }}>⚠️</span>
          <div>
            <div style={{ fontWeight: 600, fontSize: 14, color: "#eab308", marginBottom: 4 }}>Profile not set up yet</div>
            <div style={{ fontSize: 13, color: "var(--text2)", lineHeight: 1.6 }}>Fill in your name and services below. OutreachOS currently uses default placeholder info in your emails. Your settings here replace it with your own details.</div>
          </div>
        </div>
      )}
      ...
      <SectionCard>
        ...
        <div style={{ fontSize: 13, color: "var(--text3)", marginBottom: 24, lineHeight: 1.6 }}>
          This information is used to personalize all emails, follow-ups, and outreach the AI generates on your behalf. It's stored only in your browser.
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
          {fields.map(f => (
            <div key={f.key}>
              <Label>{f.label}</Label>
              <input className="inp" value={local[f.key] || ""} onChange={e => update(f.key, e.target.value)} placeholder={f.placeholder} />
              <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 5 }}>{f.hint}</div>
            </div>
          ))}
        </div>
        ...
        {textareas.map(f => (
          <div key={f.key} style={{ marginBottom: 20 }}>
            <Label>{f.label}</Label>
            <textarea className="inp" value={local[f.key] || ""} onChange={e => update(f.key, e.target.value)} placeholder={f.placeholder} style={{ minHeight: 80, resize: "vertical", lineHeight: 1.7 }} />
            <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 5 }}>{f.hint}</div>
          </div>
        ))}
        ...
        <div className="row" style={{ flexWrap: "wrap", gap: 10 }}>
          ...
          <button className="btn btn-secondary" onClick={() => setLocal({ ...profile })} style={{ fontSize: 13 }}>
            Discard changes
          </button>
        </div>
      </SectionCard>
      ...
      {(local.name || local.role) && (
        <SectionCard>
          ...
          <div style={{ fontSize: 13, color: "var(--text3)", marginBottom: 16 }}>This is how your profile will appear in outreach emails:</div>
          <div style={{ background: "var(--bg3)", borderRadius: 10, padding: 20, border: "1px solid var(--border)" }}>
            <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 4 }}>{local.name || "Your Name"}</div>
            <div style={{ fontSize: 13, color: "var(--text2)", marginBottom: 4 }}>{local.role || "Your Role"}</div>
            {local.location && <div style={{ fontSize: 13, color: "var(--text3)", marginBottom: 8 }}>📍 {local.location}</div>}
            {local.services && (
              <div style={{ fontSize: 12, color: "var(--text3)", lineHeight: 1.6, marginTop: 8, paddingTop: 8, borderTop: "1px solid var(--border)" }}>
                <span style={{ fontWeight: 500, color: "var(--text2)" }}>Services: </span>{local.services}
              </div>
            )}
          </div>
        </SectionCard>
      ...
      <SectionCard style={{ borderColor: "rgba(239,68,68,.2)" }}>
        <div style={{ fontFamily: "var(--fd)", fontWeight: 700, fontSize: 16, marginBottom: 8, color: "var(--red)" }}>Danger Zone</div>
        <div style={{ fontSize: 13, color: "var(--text3)", marginBottom: 16 }}>Reset your API key to switch to a different Anthropic account.</div>
        <button className="btn btn-secondary" onClick={onReset} style={{ borderColor: "rgba(239,68,68,.3)", color: "var(--red)", fontSize: 13 }}>
          Reset API key & log out
        </button>
      </SectionCard>
    </div>
  );
...
export default function App() {
  ...
  const [profile, setProfile] = useState(() => { try { return { ...DEFAULT_PROFILE, ...JSON.parse(localStorage.getItem("oos-profile") || "{}") }; } catch { return DEFAULT_PROFILE; } });
  ...
  useEffect(() => { localStorage.setItem("oos-profile", JSON.stringify(profile)); }, [profile]);

  if (!apiKey) return <ApiKeyGate onKey={k => { const clean = k.trim(); localStorage.setItem("oos-key", clean); setApiKey(clean); }} />;
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "24px 20px", position: "relative", zIndex: 1 }}>
        ...
        <div style={{ minHeight: 400 }}>
          {view === "agent" && <AgentPanel apiKey={apiKey} profile={profile} onSent={e => setLog(p => [e, ...p])} templates={templates} />}
          {view === "batch" && <BatchPanel apiKey={apiKey} profile={profile} onBatchDone={entries => setLog(p => [...entries, ...p])} />}
          {view === "contacts" && <ContactsPanel apiKey={apiKey} />}
          {view === "templates" && <TemplatesPanel templates={templates} setTemplates={setTemplates} />}
          {view === "tracker" && <TrackerPanel apiKey={apiKey} profile={profile} log={log} setLog={setLog} onGoAgent={() => setView("agent")} />}
          {view === "analytics" && <AnalyticsPanel log={log} />}
          {view === "settings" && <SettingsPanel profile={profile} setProfile={setProfile} onReset={() => { localStorage.removeItem("oos-key"); setApiKey(""); }} />}
        </div>
            ...
            <div className="row" style={{ gap: 12 }}>
              ...
              <div>
                <div style={{ fontWeight: 600, fontSize: 14, marginBottom: 2 }}>
                  Built by{" "}
                  {profile.linkedin ? (
                    <a href={profile.linkedin} target="_blank" rel="noreferrer" style={{ color: "var(--green)", textDecoration: "none", fontWeight: 700 }}>{profile.name || "You"}</a>
                  ) : (
                    <span style={{ color: "var(--green)", fontWeight: 700 }}>{profile.name || "You"}</span>
                  )}
                </div>
                <div style={{ fontSize: 12, color: "var(--text3)" }}>{profile.role || "Freelance Specialist"}</div>
              </div>
            </div>
